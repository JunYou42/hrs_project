...
# File description #
# This file includes dance routine 3: Exotic Dance
#       Client.cpp call service 'Service_dance_routine_3', an already
...
IP="10.152.246.134"
PORT=9559
BEHAVIOR = 'dance3_stripper-e4d693/dance3_0128'
pi = 3.1415926
a = 0

## initialize server
def task_server():
    rospy.init_node('server_dance_3') # create server node
    ## Routine 1 
    s = rospy.Service('Service_dance_routine_3', MoveJoints, handle) # SERVER HANDLER
...
def handle(req):
    ...
    ## call behavior Dance 3
    print(' START call_dance3.')
    call_dance3()
...
def fall_diagnose():
    ...
    memoryProxy.subscribeToEvent("robotHasFallen","pythonModule", "recover")
...
def call_dance3():
    ...
    managerProxy = ALProxy("ALBehaviorManager", IP, 9559) # MODIFY
...
def launchAndStopBehavior(managerProxy, behaviorName):
    ...
    if (managerProxy.isBehaviorInstalled(behaviorName)):
        ...
        if (not managerProxy.isBehaviorRunning(behaviorName)):
            ...
            print "Behavior stripper dance is launched."
            time.sleep(0.5)
        else:
